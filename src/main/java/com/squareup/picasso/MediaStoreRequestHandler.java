...
class MediaStoreRequestHandler extends ContentStreamRequestHandler {
    private static final String[] CONTENT_ORIENTATION = new String[]{ParametersKeys.ORIENTATION};
    ...
    public Result load(Request data, int networkPolicy) throws IOException {
        ...
        if (data.hasSize()) {
            ...
            if (!isVideo && picassoKind == PicassoKind.FULL) {
                return new Result(decodeContentStream(data), LoadedFrom.DISK, exifOrientation);
            }
            Bitmap bitmap;
            long id = ContentUris.parseId(data.uri);
            Options options = RequestHandler.createBitmapOptions(data);
            options.inJustDecodeBounds = true;
            RequestHandler.calculateInSampleSize(data.targetWidth, data.targetHeight, picassoKind.width, picassoKind.height, options, data);
            if (isVideo) {
            ...
            if (bitmap != null) {
                return new Result(bitmap, LoadedFrom.DISK, exifOrientation);
            }
        }
        return new Result(decodeContentStream(data), LoadedFrom.DISK, exifOrientation);
    }
