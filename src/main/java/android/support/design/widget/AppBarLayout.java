        ...
        private int getOverlapForOffset(View dependency, int offset) {
            if (this.mOverlayTop != 0 && (dependency instanceof AppBarLayout)) {
                ...
                if (availScrollRange != 0) {
                    return MathUtils.constrain(Math.round((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT + (((float) offset) / ((float) availScrollRange))) * ((float) this.mOverlayTop)), 0, this.mOverlayTop);
                }
            }
